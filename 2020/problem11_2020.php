...
<head>
    ...
    <meta name="title" content="GATE-MA-2020 Problem 11">
    <meta name="description" content="Let $C[0,1]$ denote the space of all real valued continuous functions on $[0,1]$ equipped with the supremum norm $\|\cdot\|_{\infty}$. Let $T:C[0,1]\to C[0,1]$ be the linear operator defined by $$T(f)(x)=\int_0^x e^{-y}f(y)dy.$$">
    <meta name="keywords" content="functional analysis, continuous functions space, compact set, operator, supremum, norm, GATE 2020 mathematics problem 11, GATE 2020 MA, gate mathematics, GATE MA, gate 2020, mathematics, Gate mathematics solution, ">
    ...
    <!-- Title -->
    <title>Problem-11 MA-2020</title>
    ...
    <!-- End of the header code -->
     ...
     <div class="container">
        <div class="jumbotron jumbotron-math">
            ...
            Let $C[0,1]$ denote the space of all real valued continuous functions on $[0,1]$ equipped with the supremum norm $\norm{\cdot}_{\infty}$. Let $T:C[0,1]\to C[0,1]$ be the linear operator defined by $$T(f)(x)=\int_0^x e^{-y}f(y)dy.$$
            Then 
            <ol type="A">
                <li>$\norm{T}=1$</li> <br>
                <li>$I-T$ is not invertible</li> <br>
                <li>$T$ is surjective</li> <br>
                <li>$\norm{I+T}=1+\norm{T}$</li>
            </ol>
            ...
            <div class="content">
                ...
                \begin{equation*}
                   \norm{T}=\sup_{||f||_{\infty}=1} ||T(f)||_{\infty}.
                \end{equation*}
                Now 
                \begin{align*}
                   \norm{T(f)}_{\infty}=& \sup_{x\in[0,1]} |T(f)(x)|\\
                   =& \sup_{x\in[0,1]} \left|\int_0^x e^{-y}f(y)dy\right|\\
                   \leq& \sup_{x\in[0,1]} \int_0^x e^{-y}|f(y)|dy\\
                   =& \int_0^1 e^{-y}|f(y)|dy\\
                   \leq& \int_0^1 e^{-y}\norm{f}_{\infty}dy\\
                   =& \left( 1-e^{-1}\right)\norm{f}_{\infty}\\
                   <& ||f||_{\infty}.
                \end{align*}
                Thus $\norm{T(f)}_{\infty}<\norm{f}_{\infty}$ and consequently $\norm{T}<1.$ So this is <strong>false</strong>.
                <br><br>

                <!-- Option B (if exists) -->
                <h3 class="options">(B) $I-T$ is not invertible</h3>
                 As $\norm{T}<1$, so $I-T$ is invertible and $(I-T){-1}=\sum_{n=0}^{\infty}T^n.$ So this is <strong>false</strong>.
                 <br><br>

                <!-- Option C (if exists) -->
                <h3 class="options">(C) $T$ is surjective</h3>
                Clearly by definition of $T(f)$ using fundamental theorem of calculus, we have for any $f\in C[0,1]$ $T(f)$ is always a differentiable function on $[0,1].$ So for any $g\in C[0,1]$ which is not differentiable on $[0,1]$ has no pre-image under $T$. Hence $T$ is not surjective and this is <strong>false</strong>.
                <br><br>

                <!-- Option D (if exists) -->
                <h3 class="options">(D) $\norm{I+T}=1+\norm{T}$</h3>
                (As all other options are false, so it should be `true'.) From the properties of operator norm we have $||I+T||\leq ||I||+||T||=1+||T||$. But if we take $f(x)=1$ for all $x\in [0,1]$, then $||f||_{\infty}=1$ and 
                \begin{align*}
                   \norm{(I+T)(f)}_{\infty}=&\sup_{x\in[0,1]} |f(x)+T(f)(x)|\\
                   =& 1+\sup_{x\in[0,1]} |T(f)(x)|\\
                   =& 1+\norm{T(f)}_{\infty}.
                \end{align*}
                Hence $\norm{I+T}=1+\norm{T}.$ So this is <strong>true</strong>.
            </div>
        <br><br>
            <a href="problem10_2020.php"><div class="button prev">$\leftarrow$ Problem 10</div></a>
            <a href="problem12_2020.php"><div class="button next">Problem 12 $\rightarrow$</div></a>
        </div>
    </div>
...
<div class="container">
    <div class="jumbotron jumbotron-custom xyz">
        ...
        <a href="problem11_2020.php" style="text-decoration: none";><button type="button" class="btn btn-primary btn-circle btn-md">11</button> </a>
